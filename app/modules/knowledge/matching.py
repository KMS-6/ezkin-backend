...
Persona에 구조화 인구통계 필드가 없으므로 population을 검증할 수 없다. 특정 집단용
Claim이 모든 사용자에게 노출되지 않도록 범용 population인 "성인"만 허용한다.
"""
...
from app.modules.knowledge.search import active_claim_versions

GENERAL_POPULATION = "성인"
...
async def find_claim(
    ...
    *,
    ...
    version을 우선한다.
    """
    allowed_claim_versions = await active_claim_versions(db)
    if not allowed_claim_versions:
        return None
    ...
    doc_result = await db.execute(
        ...
        .where(
            ...
            KnowledgeDocument.claim_id.in_(allowed_claim_versions),
            KnowledgeDocument.topic == topic,
        )
        .order_by(KnowledgeDocument.claim_version.desc())
    )
    for document in doc_result.scalars():
        if document.claim_id is None or document.claim_version is None:
            continue
        if allowed_claim_versions.get(document.claim_id) != document.claim_version:
            continue
        if document.population != GENERAL_POPULATION:
            continue
        ...
        if not required or not required.issubset(facts):
            continue
        if not document.allowed_expressions:
            continue
        return ClaimMatch(
