...
|---|---|---|---|---|
| Sleep and Skin Barrier — NCBI PMC 2022 | [PMC8775463](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8775463/) | `draft` (0017에서 되돌림, 원래 등록 시 `approved`) | `sleep` | Claim Card `claim_sleep_barrier_001` — 검토 완료 전까지 미검색 |
...
| Ambient Humidity and the Skin — PubMed 2016 | [PubMed 27306376](https://pubmed.ncbi.nlm.nih.gov/27306376/) | `draft` (0017에서 되돌림, 원래 등록 시 `approved`) | `humidity` | Claim Card `claim_humidity_dryness_001` — 검토 완료 전까지 미검색 |
...
로컬 Postgres(`docker compose up -d db` → `alembic upgrade head`)에서 직접 확인함. **아래는 0016까지 적용했을 때(approved 상태)의 결과이며, 0017 적용 후에는 `find_claim(topic="sleep"/"humidity")` 모두 `None`을 반환한다 — 매칭 로직 자체는 정상 동작함을 확인했다는 기록으로만 남긴다.**
