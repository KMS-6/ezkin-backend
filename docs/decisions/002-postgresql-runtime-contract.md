...
# ADR 002: 운영 데이터베이스를 PostgreSQL로 강제한다
...
- `AAC_ENVIRONMENT=production`에서는 명시적인 PostgreSQL `AAC_DATABASE_URL`을 필수로 한다.
...
- 환경 종류를 나타내는 `AAC_ENVIRONMENT` 설정을 배포 환경마다 관리해야 한다.
