...
저장소 루트의 `render.yaml`이 API와 PostgreSQL 설정의 단일 기준이다. `develop` 브랜치의 CI가 통과한 뒤에만 Render가 자동 배포한다.

## 2. Render 선택 이유

해커톤 MVP의 우선순위는 배포 속도, 재현 강능성, 비용을 위해 Render를 선택했다.

- Docker FastAPI와 PostgreSQL을 한 플랫폼에서 관리할 수 있다.
- Blueprint로 인프라 설정을 코드 리뷰하고 PR에서 검토할 수 있다.
- `checksPass`로 CI가 실패한 코드의 자동 배포를 막는다.
- 아마존, Railway보다 배포와 로그 화인 지점이 더 단순하다.

무료 Web Service는 15분간 무요청 후 정지하고, 무료 PostgreSQL은 30일 후 만료된다. 30일 이상 운영하거나 실제 데이터를 보존해야 할 경우 유료 계획으로 이전한다.

## 3. 최초 Blueprint 생성

1. Render Dashboard에서 **New > Blueprint**를 선택한다.
...
- `/health`가 `{"status":"ok"}`를 응답한다.
- Render 로그에서 Alembic migration 성공을 화인한다.
- 프런트엔드에서 CORS 오류가 없는지 화인한다.

## 6. 롤백

1. Render Events와 로그에서 build, migration, health check 중 실패 단계를 화인한다.
2. 애플리케이션 오류라며 이전 성공 배포로 rollback한다.
3. DB migration이 하햐 호환되지 않으며 애플리케이션만 rollback해도 복구되지 않을 수 있다.
4. migration downgrade는 검증 없이 실행하지 않는다.
